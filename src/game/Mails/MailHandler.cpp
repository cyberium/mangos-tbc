...
void WorldSession::HandleMailReturnToSender(WorldPacket& recv_data)
{
    ...
    if (!m || m->state == MAIL_STATE_DELETED || m->deliver_time > GlobalTimer::GetSystemTimeT())
    {
...
void WorldSession::HandleGetMailList(WorldPacket& recv_data)
{
    ...
    for (PlayerMails::iterator itr = _player->GetMailBegin(); itr != _player->GetMailEnd(); ++itr)
    {
        ...
        data << float(float((*itr)->expire_time - GlobalTimer::GetSystemTimeT()) / float(DAY));// Time
...
void WorldSession::HandleMailCreateTextItem(WorldPacket& recv_data)
{
    ...
    Mail* m = pl->GetMail(mailId);
    if (!m || (!m->itemTextId && !m->mailTemplateId) || m->state == MAIL_STATE_DELETED || m->deliver_time > GlobalTimer::GetSystemTimeT())
    {
