...
bool GroupLootRoll::TryToStart(LootBase& loot, LootItemSPtr& lootItem)
{
    if (!m_isStarted)
    {
        m_isStarted = true;
        ...
        for (auto guid : m_loot->GetOwnerSet())
        {
            Player* plr = sObjectMgr.GetPlayer(guid);
            if (!plr || !m_lootItem->IsAllowed(guid))       // check if player meet the condition to be able to roll this item
            {
                m_rollVoteMap[guid].vote = ROLL_NOT_VALID;
                continue;
            }
            m_rollVoteMap[guid].vote = ROLL_NOT_EMITED_YET; // initialize player vote map
            ++playerCount;
        }
        ...
        if (playerCount > 1)                                // check if more than one player can loot this item
        {
            // start the roll
            SendStartRoll();
            m_rollTimer.Reset(LOOT_ROLL_TIMEOUT);
            return true;
        }
...
bool GroupLootRoll::UpdateRoll(uint32 diff)
{
    RollVoteMap::const_iterator winnerItr = m_rollVoteMap.end();
    m_rollTimer.Update(diff);
    if (AllPlayerVoted(winnerItr) || m_rollTimer.Passed())
    {
        Finish(winnerItr);
        return true;
    }
...
void GroupLootRoll::SendStartRoll()
{
    ...
    data << m_loot->GetLootTarget()->GetObjectGuid();       // creature guid what we're looting
    data << uint32(m_lootItem->lootSlot);                   // item slot in loot
    ...
    for (RollVoteMap::const_iterator itr = m_rollVoteMap.begin(); itr != m_rollVoteMap.end(); ++itr)
    {
        ...
        if (m_loot->GetLootMethod() == NEED_BEFORE_GREED && plr->CanUseItem(m_lootItem->itemProto) != EQUIP_ERR_OK)
...
void GroupLootRoll::Finish(RollVoteMap::const_iterator& winnerItr)
{
    ...
    else
    {
        ...
        if (plr && plr->GetSession())
        {
            m_loot->SendItem(*plr, m_lootItem);
        }
        else
        {
