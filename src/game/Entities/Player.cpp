...
void Player::Update(uint32 update_diff, uint32 p_time)
{
    ...
    {
        m_createdInstanceClearTimer = MINUTE * IN_MILLISECONDS;
        UpdateNewInstanceIdTimers(Clock::now());
    }
    else
...
AreaLockStatus Player::GetAreaTriggerLockStatus(AreaTrigger const* at, uint32& miscRequirement)
{
    ...
    if (mapEntry->IsDungeon() && ((map && map->GetDifficulty() == DUNGEON_DIFFICULTY_NORMAL) || (mapEntry->IsDungeon() && GetDifficulty() == DUNGEON_DIFFICULTY_NORMAL)))
    {
...
{
    if (m_enteredInstances.find(instanceId) == m_enteredInstances.end())
        m_enteredInstances.emplace(instanceId, Clock::now() + std::chrono::hours(1));
}
...
void Player::_LoadCreatedInstanceTimers()
{
    ...
    if (result)
    {
        ...
        {
            Field* fields = result->Fetch();
            TimePoint expireTime = Clock::from_time_t(fields[0].GetUInt64());
